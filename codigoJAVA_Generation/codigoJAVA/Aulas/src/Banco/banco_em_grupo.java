...
public class banco_em_grupo {
	...
	static void contaCorrente (double saldo) {
		int talao[] = new int[10];
		char opcao;
		double valor;
		int somaTalao = 0;
		...
		do {
			for(int i=0; i<3;i++) {
				...
				System.out.println("[D]-DÉBITO \\ [C]-CRÉDITO \\ [T]-SOLICITAR TALÃO(MAX 3 POR MÊS)");
				opcao = t.next().toUpperCase().charAt(0);
				...
				if(opcao=='D') {	
					...
					System.out.println("VOCÊ POSSUI:"+qtdTalao+" DISPONÍVEIS");
					
					for(int t =0;t<10;t++)	{						
						if(qtdTalao > 0) {
							System.out.println("DESEJA SACAR? S/N");
							escolha = banco_em_grupo.t.next().toUpperCase().charAt(0);
							System.out.println("QUANTOS?");
							qtdEscolha = banco_em_grupo.t.nextInt();
							if(qtdEscolha<=qtdTalao ) {
								qtdTalao -= qtdEscolha;
								System.out.println("OPERAÇÃO CONCLUIDA");
								talao[i] = qtdTalao;
							}
							else {
								System.out.println("MÁXIMO DE 3 TALÕES ATINGIDO");
							}
							
							
						}
						else {
							System.out.println("VOCÊ NÃO POSSUI MAIS TALÕES");
						}
						
					}
				}
				else {
					System.out.println("Insira um comando válido!! [D]-DÉBITO \\\\ [C]-CRÉDITO \\\\ [T]-SOLICITAR TALÃO(MAX 3 POR MÊS)");
	...
	static void contaEspecial (double saldo)
	{
	...
	static void contaUniversitaria ( double saldo) {
		char opcao;
		double valor;
		double limite=1000;
